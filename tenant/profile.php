...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        setFlash('success', "Profil dan foto profil Anda berhasil diperbarui!");
        header("Location: profile.php");
        exit;
    }
    ...
    if ($action === 'change_password') {
        ...
        if (empty($currentPassword) || empty($newPassword) || empty($confirmPassword)) {
            ...
            if ($userRow && password_verify($currentPassword, $userRow['password'])) {
                ...
                setFlash('success', "Kata sandi akun Penyewa berhasil diperbarui!");
            } else {
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white font-heading">Profil & Foto Penyewa</h2>
        <p class="text-slate-500 dark:text-slate-400 text-xs mt-1">Kelola data kontak pribadi, perbarui foto profil, dan ubah kata sandi akun hunian Anda.</p>
    </div>
</div>
    ...
    <div class="lg:col-span-7 p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-6 shadow-sm">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800/80">
                ...
                <div class="relative group flex-shrink-0 cursor-pointer" onclick="openAvatarZoomModal('<?= htmlspecialchars($avatarUrl) ?>', '<?= htmlspecialchars($userData['name']) ?>')" title="Klik untuk memperbesar foto">
                    <div class="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl bg-slate-200 dark:bg-slate-800 relative transition-transform duration-300 group-hover:scale-105">
                        <img id="avatarPreviewImg" src="<?= htmlspecialchars($avatarUrl) ?>" alt="Foto Profil <?= htmlspecialchars($userData['name']) ?>" class="w-full h-full object-cover">
                ...
                <div class="flex-1 space-y-2 text-center sm:text-left">
                    <h4 class="text-base font-bold text-slate-900 dark:text-white font-heading">Foto Profil Anda</h4>
            ...
            <div class="space-y-4 pt-2">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 mt-1">Email digunakan sebagai identitas akun login dan tidak dapat diubah sendiri.</p>
                </div>
    ...
    <div class="lg:col-span-5 p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-6 shadow-sm h-fit">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-xs text-slate-500 dark:text-slate-400">Amankan akun hunian Anda secara berkala</div>
            </div>
        </div>
...
function openAvatarZoomModal(imageUrl, userName) {
    ...
    // Use current preview image src if changed
...
function closeAvatarZoomModal(e) {
    const modal = document.getElementById('avatarZoomModal');
